...
import com.alipay.config.VerifyEnum;
...
public class AlipayNotify {
	...
	public static boolean verify(Map<String, String> params) {
		...
		if (Integer.parseInt(params.get("verify")) == VerifyEnum.KEY.value()) {
			isSign = getSignVeryfy(params, sign);
		} else {
			isSign = getRSASignVerify(params, sign, platform);
		}
...
//					AlipayConfig.input_charset);
			...
			isSign=MD5.verify(preSignStr, 
					sign,paramPlatformModel.getKey(), 
					paramPlatformModel.getInput_charset());
		}
		return isSign;
	}
	 ...
	 * @return 生成的签名结果
	 */
	private static boolean getRSASignVerify(Map<String, String> Params,
			String sign, int platform) {
		// 过滤空值、sign与sign_type参数
		Map<String, String> sParaNew = AlipayCore.paraFilter(Params);
		// 获取待签名字符串
		String preSignStr = AlipayCore.createLinkString(sParaNew);
