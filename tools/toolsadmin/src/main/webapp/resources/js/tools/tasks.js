...
function initModal(modal,button){
	...
	if (operateType == 1) { // 新增
		...
		modal.find("[name=selPriorityLevel]").val(modal.find("[name=selPriorityLevel]").find("option:first").val());
	} else if (operateType == 2) { // 编辑
		var $li = button.parents("li");
        var id = $li.data("id");
        modal.find("[name=id]").val(id);
		$.get(window.basePath+"/tools/tasks/getbyid",{id:id},function(resp){
			if(!resp.iserror){
				modal.find("[name=title]").val(resp.data.title);
				modal.find("[name=content]").val(resp.data.content);
				modal.find("[name=selPriorityLevel]").val(resp.data.priorityLevel);
