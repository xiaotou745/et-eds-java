...
public class BusinessHttpService implements IBusinessHttpService {
	...
	public HttpResultModel<Integer> getPushOrderType(GetPushOrderTypeReq req) {
		...
		if (req.getBusinessId() == 0 || type == null) {
			return result.setStatus(
					GetPushOrderTypeReturnEnum.BusinessIdError.value())
					.setMessage(
							GetPushOrderTypeReturnEnum.BusinessIdError.desc());
		}
	 ...
	 */
	@Override
	public HttpResultModel<GetMyServiceClientersResp> getMyServiceClienters(
			PagedGetMyServiceClientersReq req) {
		HttpResultModel<GetMyServiceClientersResp> result = new HttpResultModel<GetMyServiceClientersResp>();
		if (req.getBusinessId() == 0) {
			return result.setStatus(
					GetMyServiceClientersReturnEnum.BusinessIdError.value())
					.setMessage(
							GetMyServiceClientersReturnEnum.BusinessIdError
									.desc());
		}
		if (req.getAuditStatus() == null || req.getAuditStatus() > 1) {
			return result.setStatus(
					GetMyServiceClientersReturnEnum.AuditStatusError.value())
					.setMessage(
							GetMyServiceClientersReturnEnum.AuditStatusError
									.desc());
		}
		result.setResult(businessClienterRelationService
				.getMyServiceClienters(req));
		return result;
	}
	 ...
	 */
	@Override
	public HttpResultModel<Object> bindClienterBusiness(
			BindClienterBusiness bindClienterBusiness) {
		HttpResultModel<Object> result = new HttpResultModel<Object>();
		if (bindClienterBusiness.getBusinessId() <= 0
				|| bindClienterBusiness.getBusinessName() == null
				|| bindClienterBusiness.getBusinessName().trim() == "") {
		...
		if (bindClienterBusiness.getClienterId() <= 0
				|| bindClienterBusiness.getClienterName() == null
				|| bindClienterBusiness.getClienterName().trim() == "") {
		...
		BusinessClienterRelation b = businessService
				.getClienterBind(bindClienterBusiness);
		if (b == null) { // 不存在绑定关系，直接绑定
			...
			cbor.setIsBind(0);
			cbor.setAuditStatus(BusinessClienterRelationAuditStatus.Wait
					.value());
			...
			if (b.getIsbind() == 0) {
				...
				boolean uprel = businessClienterRelationService
						.updateClienterBindRelation(modifycbor);
				if (uprel) {
					result.setStatus(ClienterBindBusinessEnum.Success.value());
					result.setMessage(ClienterBindBusinessEnum.Success.desc());
				} else {
	...
	public HttpResultModel<Object> optBindClienter(OptBindClienterReq req) {
		...
		if (req.getBusinessId() <= 0) {
			return res.setStatus(
					OptBindClienterReturnEnum.BusinessIdError.value())
					.setMessage(
							OptBindClienterReturnEnum.BusinessIdError.desc());
		}
		if (req.getAuditStatus() != BusinessClienterRelationAuditStatus.Pass
				.value()
				&& req.getAuditStatus() != BusinessClienterRelationAuditStatus.Refuse
						.value()) {
			return res.setStatus(
					OptBindClienterReturnEnum.AuditStatusError.value())
					.setMessage(
							OptBindClienterReturnEnum.AuditStatusError.desc());
		}
		if (req.getRelationId() <= 0) {
			return res.setStatus(
					OptBindClienterReturnEnum.RelationIdError.value())
					.setMessage(
							OptBindClienterReturnEnum.RelationIdError.desc());
		}
		return businessClienterRelationService.optBindClienter(req) <= 0 ? res
				.setStatus(OptBindClienterReturnEnum.StatusError.value())
				.setMessage(OptBindClienterReturnEnum.StatusError.desc()) : res;
	}
	...
	public HttpResultModel<Object> removeRelation(ClienterBindOptionReq req) {
		...
		if (req.getBusinessId() <= 0) {
			return res
					.setStatus(RemoveRelationReturnEnum.BusinessIdError.value())
					.setMessage(RemoveRelationReturnEnum.BusinessIdError.desc());
		}
		if (req.getClienterId() <= 0) {
			return res
					.setStatus(RemoveRelationReturnEnum.ClienterIdError.value())
					.setMessage(RemoveRelationReturnEnum.ClienterIdError.desc());
		}
		if (req.getRemark() == null || req.getRemark().isEmpty()
				|| req.getRemark().length() < 5
				|| req.getRemark().length() > 100) {
			return res.setStatus(RemoveRelationReturnEnum.RemarkError.value())
					.setMessage(RemoveRelationReturnEnum.RemarkError.desc());
		}
	...
	public HttpResultModel<MyOrderDetailBResp> getMyOrderDetailB(
		...
		result.setStatus(HttpReturnRnums.Success.value());
		result.setMessage(HttpReturnRnums.Success.desc());
		MyOrderDetailBResp orderDetailBResp = businessService
				.getMyOrderDetailB(orderGrabBReq);
	...
	public HttpResultModel<BusinessBasicInfoModel> getBusinessInfo(
		...
		result.setStatus(HttpReturnRnums.Success.value());
		result.setMessage(HttpReturnRnums.Success.desc());
		BusinessBasicInfoModel businessBasicInfoModel = businessService
				.getBusinessInfo(businessReq);
	...
	public HttpResultModel<Object> sendCode(BSendCodeReq req) {
		...
		if (phoneNo == null || phoneNo.isEmpty()
				|| !CommonValidator.validPhoneNumber(phoneNo)) {
			return res.setStatus(SendSmsReturnType.PhoneError.value())
					.setMessage(SendSmsReturnType.PhoneError.desc());
		} else if (req.getMessageType() != 0 && req.getMessageType() != 1) { // 修改绑定手机号验证新手机号
			return res.setStatus(SendSmsReturnType.MessageTypeError.value())
					.setMessage(SendSmsReturnType.MessageTypeError.desc());
		}
		try {
			if (req.getType() == BSendCodeType.Register.value()) {
				key = String.format(RedissCacheKey.PostRegisterInfo_B, phoneNo);
				Content = "短信验证码：#验证码#（请完成注册），如非本人操作，请忽略本短信。";
			} else if (req.getType() == BSendCodeType.ForgetPwd.value()
					|| req.getType() == BSendCodeType.ModifyPwd.value()) {
				key = String.format(RedissCacheKey.CheckCodeFindPwd_B, phoneNo);
				Content = "短信验证码：#验证码#（请完成修改密码），如非本人操作，请忽略本短信。";
			} else if (req.getType() == BSendCodeType.ModifyPhone.value()) { // 修改绑定手机号验证当前手机号
				key = String.format(
						RedissCacheKey.Business_SendCode_ModifyPhone, phoneNo);
				Content = "短信验证码：#验证码#(请完成验证)，此验证码5分钟内有效，如非本人操作，请忽略本短信。";
			} else if (req.getType() == BSendCodeType.ModifyPhoneNewPhone
					.value()) { // 修改绑定手机号验证新手机号
				key = String.format(
						RedissCacheKey.Business_SendCode_ModifyPhoneNewPhone,
						phoneNo);
				Content = "短信验证码：#验证码#(请完成新账号验证)，此验证码5分钟内有效，如非本人操作，请忽略本短信。";
			} else if (req.getType() == BSendCodeType.NoLoginPhone.value()) {
				key = String.format(RedissCacheKey.PostRegisterInfo_B, phoneNo);
				// 如果存在
				if (businessService.isExist(phoneNo)) {
					Content = "短信验证码：#验证码#，如非本人操作，请忽略本短信。";
				} else {
					Content = "短信验证码：#验证码#，该验证码为登录密码，如非本人操作，请忽略本短信。";
				}
			}
			if (key == "") {
				return res.setStatus(SendSmsReturnType.Fail.value())
						.setMessage(SendSmsReturnType.Fail.desc());// 发送失败
			}
			String code;// 获取随机数
			if (req.getMessageType() == 1) { // 语音验证码
				...
				if (obj == null || obj.isEmpty()) {
					return res.setStatus(
							SendSmsReturnType.CodeNotExists.value())
							.setMessage(SendSmsReturnType.CodeNotExists.desc());
				}
				String keycheck = key + "_voice";
				if (ParseHelper.ToInt(redisService.get(keycheck, String.class)) == 1) {
					return res.setStatus(SendSmsReturnType.Sending.value())
							.setMessage(SendSmsReturnType.Sending.desc());
				}
			...
			long resultValue = req.getMessageType() == 0 ? SmsUtils.sendSMS(
					phoneNo, Content) : SmsUtils.sendVoiceSMS(phoneNo, Content);
			if (resultValue <= 0) {
				return res.setStatus(SendSmsReturnType.Fail.value())
						.setMessage(SendSmsReturnType.Fail.desc());// 发送失败
			}
			return res.setStatus(SendSmsReturnType.Success.value()).setMessage(
					SendSmsReturnType.Success.desc());// 设置成功
	...
	public HttpResultModel<Object> businessModiyPhoneStep1(BCheckCodeReq req) {
		...
		if (req.getPhoneNo() == null || req.getPhoneNo().isEmpty()
				|| CommonValidator.validPhoneNumber(phoneNo)) {
			return res.setStatus(
					BusinessModiyPhoneReturnEnum.PhoneError.value())
					.setMessage(BusinessModiyPhoneReturnEnum.PhoneError.desc());
		}
		if (req.getCode() == null || req.getCode().isEmpty()) {
			return res
					.setStatus(BusinessModiyPhoneReturnEnum.VerCodeNull.value())
					.setMessage(BusinessModiyPhoneReturnEnum.VerCodeNull.desc());
		}
		if (redisService.get(String.format(
				RedissCacheKey.Business_SendCode_ModifyPhone, phoneNo),
				String.class) == null) {
			return res
					.setStatus(BusinessModiyPhoneReturnEnum.CodeTimeOut.value())
					.setMessage(BusinessModiyPhoneReturnEnum.CodeTimeOut.desc());// 验证码不不能为空
		}
		if (!redisService.get(
				String.format(RedissCacheKey.Business_SendCode_ModifyPhone,
						phoneNo), String.class).equals(req.getCode())) {
			return res
					.setStatus(BusinessModiyPhoneReturnEnum.CodeError.value())
					.setMessage(BusinessModiyPhoneReturnEnum.CodeError.desc());// 发送失败
		}
		return res;
	}
	...
	public HttpResultModel<Object> businessModiyPhoneStep2(BCheckCodeReq req) {
		...
		if (req.getBusinessId() == null || req.getBusinessId() == 0) {
			return res.setStatus(
					BusinessModiyPhoneReturnEnum.BusinessId.value())
					.setMessage(BusinessModiyPhoneReturnEnum.BusinessId.desc());
		}
		if (req.getPhoneNo() == null || req.getPhoneNo().isEmpty()
				|| CommonValidator.validPhoneNumber(req.getPhoneNo())) {
			return res.setStatus(
					BusinessModiyPhoneReturnEnum.PhoneError.value())
					.setMessage(BusinessModiyPhoneReturnEnum.PhoneError.desc());
		}
		...
		System.out.println(String.format(
				RedissCacheKey.Business_SendCode_ModifyPhoneNewPhone, phoneNo));
		if (redisService.get(String.format(
				RedissCacheKey.Business_SendCode_ModifyPhoneNewPhone, phoneNo),
				String.class) == null) {
			return res
		...
		if (businessService.isExist(req.getPhoneNo())) {
			return res
					.setStatus(BusinessModiyPhoneReturnEnum.PhoneExists.value())
					.setMessage(BusinessModiyPhoneReturnEnum.PhoneExists.desc());
		}
		return businessService.businessModiyPhone(req) ? res : res.setStatus(
				BusinessModiyPhoneReturnEnum.ModifyError.value()).setMessage(
				BusinessModiyPhoneReturnEnum.ModifyError.desc());
	}
	...
	public HttpResultModel<Object> register(BusinessRegisterReq req) {
		HttpResultModel<Object> res = new HttpResultModel<Object>();
		res.setMessage(HttpReturnRnums.Success.desc()).setStatus(
				HttpReturnRnums.Success.value());
		// 手机号验证
		if (req.getPhoneNo() == null || req.getPhoneNo().isEmpty()
				|| CommonValidator.validPhoneNumber(req.getPhoneNo())) {
		...
		String key = String.format(RedissCacheKey.RegisterCount_B,
				req.getPhoneNo());
		int excuteCount = redisService.get(key, Integer.class);
		if (excuteCount >= 10) {
			return res.setMessage(RegisterBEnum.CountError.desc()).setStatus(
					RegisterBEnum.CountError.value());
		}
		redisService.set(key, excuteCount + 1, 5 * 60);
		// 获取缓存中的验证码
		String code = redisService.get(
				String.format(RedissCacheKey.PostRegisterInfo_B,
						req.getPhoneNo()), String.class);
		if (code.toLowerCase() != req.getCode().toLowerCase()) {
			return res.setMessage(RegisterBEnum.CodeError.desc()).setStatus(
					RegisterBEnum.CodeError.value());
		}
		// 密码验证
		...
		// 验证手机号是否已注册
		if (businessService.isExist(req.getPhoneNo())) {
			return res.setStatus(RegisterBEnum.PhoneExists.value()).setMessage(
					RegisterBEnum.PhoneExists.desc());
		}
