...
public class ClienterService implements IClienterService {
	...
	@Override
	public PagedResponse<BusinessClientersModel> getBusinessClienters(
	...
	public void updateCAccountBalance(ClienterMoney clienterMoney) {
		...
		clienterBalanceRecord.setRecordtype((short) clienterMoney
				.getRecordType()); //
		clienterBalanceRecord.setOperator(clienterMoney.getOperator()); // 商家id
		clienterBalanceRecord.setWithwardid((long) clienterMoney
				.getWithwardId()); // 关联单id
	...
	public void updateCAllowWithdrawPrice(ClienterMoney clienterMoney) {
		...
		clienterAllowWithdrawRecord
				.setClienterid(clienterMoney.getClienterId());// 商户Id
		clienterAllowWithdrawRecord.setAmount(clienterMoney.getAmount());
		clienterAllowWithdrawRecord
				.setStatus((short) clienterMoney.getStatus()); // 流水状态
		clienterAllowWithdrawRecord.setRecordtype((short) clienterMoney
				.getRecordType()); //
		clienterAllowWithdrawRecord.setOperator(clienterMoney.getOperator()); // 商家id
		clienterAllowWithdrawRecord.setWithwardid((long) clienterMoney
				.getWithwardId()); // 关联单id
		clienterAllowWithdrawRecord
				.setRelationno(clienterMoney.getRelationNo()); // 关联单号
	 ...
	 */
	@Override
	public PagedResponse<ClienterBindInfoModel> getClienterList(
			PagedClienterSearchReq req) {
	...
	public MyBusinessResp getMyBusiness(MyBusinessReq myBusinessReq) {
		...
		List<MyBusinessModel> myBusinessModelList = clienterDao
				.getMyBusiness(myBusinessReq);
		myBusinessResp.setMyBusinessModelList(myBusinessModelList);
		MyBusinessResp myBusinessRespTotal = clienterDao
				.getServiceBusinessTotal(myBusinessReq);
		myBusinessResp.setFuWuZhongTotal(myBusinessRespTotal
				.getFuWuZhongTotal());
		myBusinessResp.setShenQingZhongTotal(myBusinessRespTotal
				.getShenQingZhongTotal());
	...
	public ClienterUserStatusResp getUserStatus(UserStatusReq req) { 
		return clienterDao.getUserStatus(req);
	}
